...
const vocabulary = {
  ...
  n4: [ { kanji: "日本", reading: "にほん", meaning: "일본" }, { kanji: "大学", reading: "だいがく", meaning: "대학" }, { kanji: "学生", reading: "がくせい", meaning: "학생" }, { kanji: "先生", reading: "せんせい", meaning: "선생님" } ],
  n3: [], n2: [], n1: []
};
...
function createVocabularyCard(word, index) {
  ...
  card.innerHTML = `
    ...
    <div class="level-stamp">${currentLevel.toUpperCase()}</div>
  `;
...
function startQuiz() {
  ...
  maxCount = Math.min(maxCount, shuffled.length); // 단어 개수가 설정보다 적을 경우 대비
  
  quizWords = shuffled.slice(0, maxCount);
  incorrectQuestions = []; // 초기화
  currentQuizIndex = 0;
  score = 0;
...
function handleQuizAnswer(clickedBtn, isCorrect, currentWord) {
  ...
  if (isCorrect) {
      ...
      incorrectQuestions.push(currentWord); // 틀린 문제 저장
